...
export default function UserDetails(){
    ...
    const {id}=useParams()
    const loadData=()=>{
        axios.get('http://localhost:8085/api/customers/'+id)
        .then(resp=>{
            setData(resp.data)
        })
    }
    ...
    const updateStatus=()=>{
        axios.put('http://localhost:8085/api/customers/'+id)
        .then(resp=>{
            loadData()
            Swal.fire({
                title: resp.data
            ...
            <table className="table table-bordered mt-4">
                <thead>
                    <tr>
                        ...
                        <th>{data?.name}</th>
                        <th rowspan={7}>
                            <img src={'http://localhost:8085/'+data?.uidphoto} style={{width:"500px"}} alt='Adhar Card' /><br/>
                            Adhar Id     
                        </th>
                    </tr>
                    <tr>
