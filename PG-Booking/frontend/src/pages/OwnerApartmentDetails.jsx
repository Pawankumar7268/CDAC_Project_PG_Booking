...
export default function OwnerApartmentDetails() {
    ...
    const deleteApartment = id => {
        axios.delete('http://localhost:8085/api/apartments/' + id)
            .then(resp => {
    ...
    const activateApartment = id => {
        axios.put('http://localhost:8085/api/apartments/' + id)
            .then(resp => {
                Swal.fire({ title: resp.data })
                navigate('/apartments')
            })
    ...
    const handleCancel = id => {
        axios.get('http://localhost:8085/api/bookings/cancel/' + id)
            .then(resp => {
                Swal.fire({ title: resp.data })
                navigate('/apartments')
            })
    ...
    useEffect(() => {
        axios.get('http://localhost:8085/api/apartments/' + id)
            .then(resp => {
                setData(resp.data)
            })
    }, [])
    useEffect(() => {
        axios.get('http://localhost:8085/api/bookings/apartments/' + data?.id)
            .then(resp => {
                setusers(resp.data)
            })
    }, [data])
                    ...
                    <div className="col-sm-5">
                        ...
                        <div className="card" style={{ width: '28rem' }}>
                            <Carousel>
                                <Carousel.Item>
                                    <img
                                        ...
                                        src={'http://localhost:8085/' + data?.photo1} height={300}
                                        alt="First slide"
                                    />
                                </Carousel.Item>
                                <Carousel.Item>
                                    <img
                                        className="d-block w-100"
                                        src={'http://localhost:8085/' + data?.photo2} height={300}
                                        alt="Second slide"
                                    />
                                </Carousel.Item>
                                ...
                                <Carousel.Item>
                                    <img
                                        className="d-block w-100"
                                        src={'http://localhost:8085/' + data?.photo3} height={300}
                                        alt="Third slide"
                                    />
                                </Carousel.Item>
                                <Carousel.Item>
                                    <img
                                        className="d-block w-100"
                                        src={'http://localhost:8085/' + data?.photo4} height={300}
                                        alt="Fourth slide"
                                    />
                                </Carousel.Item>
                            </Carousel>
                            <div className="card-body">
                                ...
                                <h6>{data?.address}, {data?.city} {data?.state}</h6>
