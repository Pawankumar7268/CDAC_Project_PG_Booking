...
export default function Card() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8085/api/apartments')
      .then(resp => {
        setData(resp.data)
      })
  }, [])
  return (
    <div className='mt-2 mx-auto' style={{ width: "100%"}}>
      <div className='row'>
        {data.map(x => (
          <div className='col-sm-4' style={{margin:"30px 0px"}}>
            <div className="card  border-2 shadow mb-2 bg-white rounded" key={x.id} style={{height:"100%"}}>
              <div className='card-header text-center'>
                <h5 className="card-title">{x.name}</h5>
                {x.address}, {x.city} {x.district} India
              </div>
              <img src={'http://localhost:8085/' + x.photo1} style={{ height: "300px" }} className="img-fluid rounded-start" alt="..." />
